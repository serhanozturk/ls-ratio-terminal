...
<div class="wrap">
  ...
  <div class="info">
    ...
    <b>METR\u0130KLER:</b><br>
    \u2022 <b>L/S Ratio</b>: Long/Short hesap oran\u0131. >1 = long bask\u0131n, <1 = short bask\u0131n.<br>
    \u2022 <b>Open Interest (OI)</b>: A\xe7\u0131k pozisyonlar\u0131n toplam USD de\u011feri. Y\xfckselen OI = yeni para giriyor; d\xfc\u015fen OI = pozisyonlar kapan\u0131yor.<br>
    \u2022 <b>Funding Rate</b>: Pozitif = longlar shortlara \xf6d\xfcyor (a\u015f\u0131r\u0131 bo\u011fa); negatif = shortlar longlara \xf6d\xfcyor (a\u015f\u0131r\u0131 ay\u0131). Genelde 8 saatte bir.<br><br>
    <b>YORUMLAMA:</b> Y\xfcksek long oran\u0131 + y\xfcksek pozitif funding = a\u015f\u0131r\u0131 bo\u011fa, d\xfczeltme riski. Y\xfcksek short oran\u0131 + negatif funding = short squeeze potansiyeli.<br><br>
    <b>YASAL UYARI:</b> Bu ara\xe7 finansal tavsiye de\u011fildir. Metrikler tek ba\u015flar\u0131na sinyal de\u011fildir; fiyat aksiyonu ve di\u011fer veri kaynaklar\u0131 ile birlikte de\u011ferlendirilmelidir.
  </div>
...
function cleanSymbol(raw) {
  // Sadece harf ve rakam b\u0131rak; ▸, bo\u015fluk, $ vb. her \u015fey gider
...
async function run() {
  ...
  // Input'u temizlenmi\u015f hale geri yaz, b\xf6ylece kullan\u0131c\u0131 ne sorguland\u0131\u011f\u0131n\u0131 g\xf6r\xfcr
...
// Otomatik yenileme: her 60 saniyede bir mevcut coin'i tekrar \xe7eker
// Sadece tab aktifse \xe7al\u0131\u015f\u0131r (arka planda iken duraklat\u0131r, kaynak israf\u0131 olmas\u0131n)
let autoRefresh = setInterval(() => {
  if (!document.hidden && lastFetch) run();
}, 60000);

document.addEventListener('visibilitychange', () => {
  // Sekmeye geri d\xf6nd\xfc\u011f\xfcnde hemen taze veri \xe7ek
  if (!document.hidden && lastFetch) run();
});
...
def safe(fn):
    """Yard\u0131mc\u0131: bir metrik fetch fonksiyonunu \xe7a\u011f\u0131r\u0131r, hata olursa None d\xf6ner.
    OI/funding hatas\u0131 t\xfcm karta d\xfc\u015fmesin diye."""
    try:
        return fn()
    except Exception:
        return None
...
def _binance_oi(sym):
    ...
    j = http_get(url)
    # openInterest \u00fcnite olarak coin cinsinden d\xf6ner; USD'ye \xe7evirmek i\xe7in mark price laz\u0131m
    qty = float(j["openInterest"])
    # Mark price
    pj = http_get(f"https://fapi.binance.com/fapi/v1/premiumIndex?symbol={sym}")
    price = float(pj["markPrice"])
    return qty * price  # USD notional

def _binance_funding(sym):
    j = http_get(f"https://fapi.binance.com/fapi/v1/premiumIndex?symbol={sym}")
    return float(j["lastFundingRate"]) * 100  # y\xfczde olarak
...
def _bybit_oi(sym):
    # Bybit ticker \xfczerinden hem OI hem funding tek istekte gelir
    ...
    if not lst:
        return None, None
    t = lst[0]
    # openInterestValue zaten USD cinsinden notional
    oi = float(t.get("openInterestValue") or 0) or None
    # If openInterestValue not provided, compute from openInterest * lastPrice
    if not oi and t.get("openInterest") and t.get("lastPrice"):
        oi = float(t["openInterest"]) * float(t["lastPrice"])
    fr = float(t["fundingRate"]) * 100 if t.get("fundingRate") else None
    return oi, fr
...
def fetch_bybit(symbol, period, limit):
    ...
    oi_usd, funding = safe(lambda: _bybit_oi(sym)) or (None, None)
...
def _okx_metrics(inst_id):
    """OKX'te OI ve funding ayr\u0131 endpoint'lerden gelir."""
    oi = None
    fr = None
    try:
        ...
        if j.get("code") == "0" and j.get("data"):
            d = j["data"][0]
            # oiCcy = coin cinsinden, oiUsd USD cinsinden olabilir; baz\u0131 versiyonlarda yok
            if d.get("oiUsd"):
                oi = float(d["oiUsd"])
            elif d.get("oiCcy"):
                # Fiyat\u0131 ayr\u0131 \xe7ek
...
def fetch_okx(symbol, period, limit):
    ...
    oi_usd, funding = safe(lambda: _okx_metrics(inst_id)) or (None, None)
...
def _bitget_metrics(sym):
    """Bitget OI ve funding ayr\u0131 endpoint'lerden."""
    oi = None
    fr = None
    try:
        ...
        if j.get("code") == "00000":
            ...
            if ol:
                # size coin cinsinden; USD i\xe7in fiyatla \xe7arpmak laz\u0131m
                qty = float(ol[0].get("size") or 0)
                # Fiyat\u0131 ticker'dan al
...
def fetch_bitget(symbol, period, limit):
    ...
    oi_usd, funding = safe(lambda: _bitget_metrics(sym)) or (None, None)
